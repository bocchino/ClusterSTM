...
void hashtable_create(unsigned size) {
  ...
  table_locks = malloc(table_size * sizeof(grt_lock_t*));
  for (i = 0; i < grt_num_procs; ++i) {
    table[i] = 
      (node_t**) grt_all_alloc(i, local_size * sizeof(node_t*));
    for (j = 0; j < local_size; ++j)
      table_locks[i*local_size+j] = grt_all_lock_alloc(i,1);
  }
}
...
void hashtable_destroy() {
  ...
  for (i = 0; i < local_size; ++i) {
    ...
    grt_free(grt_id, table_locks[grt_id*local_size+i]);
  }
  free(table);
  free(table_locks);
}
...
#define READ_LOCK \
  grt_lock_t *lock = table_locks[hash.proc*local_size+hash.offset]; \
...
#if NB
  gasnet_put_nbi(hash.proc, (grt_word_t*) &q->next, (grt_word_t*) &p,
		 sizeof(grt_word_t));

...
foreach(@lock_suffixes) {
    push (@targets, "hashset-locks".$_);
}

foreach(@stm_suffixes) {
    push (@targets, "hashset-stm-distrib".$_);
}
...
foreach(@stm_suffixes) {
  ...
  print "hashset-stm-distrib$suffix: driver$suffix.o $lib_dir/hashset-stm-distrib${suffix}.o ${libs}\n\t\$(LINK) \$+ -o \$\@ \$(LD_FLAGS) \$(GASNET_LIBS)\n";
  print "$lib_dir/hashset-stm-distrib$suffix.o: $lib_dir/hashset-stm-distrib.c\n\tmake -C $lib_dir hashset-stm-distrib$suffix.o\n";
}
...
foreach(@lock_suffixes) {
  ...
  $libs = "\$(GRT_LIB)";
  print "hashset-locks$suffix: driver$suffix.o $lib_dir/hashset-locks${suffix}.o ${libs}\n\t\$(LINK) \$+ -o \$\@ \$(LD_FLAGS) \$(GASNET_LIBS)\n";
  print "$lib_dir/hashset-locks$suffix.o: $lib_dir/hashset-locks.c\n\tmake -C $lib_dir hashset-locks$suffix.o\n";
}

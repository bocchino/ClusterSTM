...
/* Standard debugging hooks for `umalloc'.
...
struct hdr {
    uintptr_t size;		/* Exact size requested by user.  */
    unsigned long int magic;	/* Magic number to check header integrity.  */
};

static void checkhdr(struct mdesc *, const struct hdr *);
static void ufree_check(void *, void *);
static void *umalloc_check(void *, uintptr_t);
static void *urealloc_check(void *, void *, uintptr_t);
...
static void *umalloc_check(void *md, uintptr_t size)
{
    struct hdr *hdr;
    struct mdesc *mdp;
    uintptr_t nbytes;
    ...
    hdr = (struct hdr *) umalloc(md, nbytes);
...
/* Turn on default checking for umalloc/urealloc/ufree, for the heap specified
   ...
   Note that we disallow installation of initial checking hooks if umalloc
   ...
   malloc calls before the user code had a chance to call umcheck or umcheckf,
   but never calls free with these values.  Thus if we are certain that only
   values obtained from mallocs after an umcheck/umcheckf will ever be passed
   ...
   call umcheck to use a different corruption handler if desired.

   Returns non-zero if checking is successfully enabled, zero otherwise. */

int umcheckf(umalloc_heap_t *md, void (*func) (void), int force)
{
    struct mdesc *mdp;
    int rtnval;
    ...
    /* If we haven't yet called umalloc the first time for this heap, or if we
       have hooks that were previously installed, then allow the hooks to be
       initialized or updated. */

    if (force ||
	!(mdp->flags & MMALLOC_INITIALIZED) || (mdp->ufree_hook != NULL)) {
	mdp->ufree_hook = ufree_check;
	mdp->umalloc_hook = umalloc_check;
	mdp->urealloc_hook = urealloc_check;
...
/* This routine is for backwards compatibility only, in case there are
   still callers to the original umcheck function. */

int umcheck(umalloc_heap_t *md, void (*func) (void))
{
    int rtnval;

    rtnval = umcheckf(md, func, 0);
    return (rtnval);
}
